...
?>
<?php
    /* Extensi�n de la clase inventarios_muestrarios */
    function donothing()
    {
        return True;
    }
	...
	$user = newObject("user");
        if ($user->getUserByName($username))
        {
            ...
            $link = "Hemos recibido una solicitud para el cambio de contrase�a para el usuario <b>{$user->username}</b>, si desea realizar este cambio, haga click en el siguiente enlace:<br><br><a href=\"{$URL}hash={$action->hash}\">{$URL}hash={$action->hash}</a><br><br>
            ...
            $mail->Subject = $SYS["SITENAME"]."::Solicitud de confirmaci�n de cambio de contrase�a";
            $mail->Body = $link;
            if (!$mail->Send())
                echo $mail->ErrorInfo();
            else
        ...
        if ($hash != "")
        {
            ...
            if ($action->nRes > 0)
            {
                ...
                if ($user->save())
                {
                    $temp->completed = 'Si';
                    $temp->save();
                    $link = "Se ha generado una nueva contrase�a para su usuario, a continuaci�n le informamos de sus nuevos datos para conectarse:<br><br>
                    ...
                    $mail->Attached = $mail->AddAttachment($path, $name = "", $encoding = "base64",
                        $type = "application/octet-stream");
                    $mail->Subject = $SYS["SITENAME"]._("::Nueva contrase�a de usuario");
                    $mail->Body = $link;
                    if (!$mail->Send()) {
