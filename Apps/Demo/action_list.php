...
class TerminalApp extends wDesktop {
    ...
    function __construct($name=null, &$parent) {
        ...
        $this->GSPAnel = new GSControlPanel($this->FormWindow, "Items", "item");
...
'{$this->GSPAnel->dForm->components["callerid"]->id}',0,'')");
	...
	function afterrequestloadFromId( $objResponse, $obj, $fjid) {
		...
		if ($obj->coreObject->properties["hasvoicemail"]=='Si') {
			//$objResponse->assign($this->GSPAnel->dForm->components["mailbox"]->id,"disabled","");
			$objResponse->assign($this->GSPAnel->dForm->components["vmsecret"]->id,"disabled","");
			$objResponse->assign($this->GSPAnel->dForm->components["email"]->id,"disabled","");
			$objResponse->assign($this->GSPAnel->dForm->components["vmcontext"]->id,"disabled","");
		} else {
			//$objResponse->assign($this->GSPAnel->dForm->components["mailbox"]->id,"disabled","true");
			$objResponse->assign($this->GSPAnel->dForm->components["vmsecret"]->id,"disabled","true");
			$objResponse->assign($this->GSPAnel->dForm->components["email"]->id,"disabled","true");
			$objResponse->assign($this->GSPAnel->dForm->components["vmcontext"]->id,"disabled","true");
		}
