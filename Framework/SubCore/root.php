...
class Ente extends core {
    ...
    function update() {
        ...
        //------------------------------------------------------------------------------------------
        $this->lastqueryA = array();
        array_walk($this->properties, "fadd", $this);
        $q = "UPDATE `{$prefix}_" . $this->name . "` SET ";
        debug("QueryAsArray: " . print_r($this->lastqueryA, true), "red");

        $q.=implode(",", $this->lastqueryA) . "  WHERE `ID`=" . $this->ID . "";
        $bdres = _query($q);
        $this->nRes = _affected_rows();
        if ($this->nRes > -1)
            return $this->ID;
