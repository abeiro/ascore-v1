    ...
    function render() {
        ...
        if (!$this->reversed)
            echo "<span style='$specialCSS' title='{$this->tooltip}'>{$this->label} </span>
                <input type='checkbox' name='{$this->name}_ctl' $eventCode id='{$this->id}_ctl'  style='{$this->cssStyle}' $addSel onclick='$(\"{$this->id}\").value=$(\"{$this->id}_ctl\").checked' {$this->disabled}> ";
        else
            echo "<input type='checkbox' name='{$this->name}_ctl' $eventCode id='{$this->id}_ctl'  style='{$this->cssStyle}' $addSel onclick='$(\"{$this->id}\").value=$(\"{$this->id}_ctl\").checked' {$this->disabled}>
