...
if ($TrazaStatus) {
	echo "\t<!-- Trace -->\n";
	echo "\t\t<script type=\"text/javascript\" language=\"JavaScript1.3\">\n";
	echo "\t\t\tdebugWdw.document.write(':: $string <br>')\n";
	echo "\t\t</script>\n";
	echo "\t<!-- Trace -->\n";
	}
}
...
echo "\t<!-- System Trace -->\n";
echo "\t\t<script type=\"text/javascript\" language=\"JavaScript1.3\">\n";
echo  "\t\t\tdebugWdw=window.open('','Debug',\"menubar=no,scrollbars=yes,resizable=yes,width=640,height=480\")\n";
echo "\t\t\tdebugWdw.document.title('".md5($_SERVER["HTTP_HOST"])."')\n";
echo "\t\t</script>\n";
echo "\t<!-- System Trace -->\n";
logDebug("<hr><b>Traza Iniciada</b> ".strftime("%H:%M")."<hr>");
	...
	if($mode == 3) {
		print '
			<script type="text/javascript" language="JavaScript">
				...
				debugwindow.document.open();
				debugwindow.document.write("unserialize(\''.str_replace("'", "\\'", addslashes( str_replace("\r\n", '\n', serialize($array) ) ) ).'\');");
				debugwindow.document.close();
				debugwindow.document.title = "Debugwindow for : http://'.$debugwindow_origin.'";
				debugwindow.focus();
			</script>
...
function arrayDump($array) {
	logDebug(print_a( $array, 1));
}

function dataDump($array) {
	echo print_a( $array, 1,True);
}

function dataDumpS($array) {
